#   Copyright (C) 2020 by ZestIOT. All rights reserved. The information 
#   in this document is the property of ZestIOT. Except as specifically 
#   authorized in writing by ZestIOT, the receiver of this document 
#   shall keep the information contained herein confidential and shall 
#   protect the same in whole or in part from disclosure and 
#   dissemination to third parties. Disclosure and disseminations to the 
#   receiver's employees shall only be made on a strict need to know 
#   basis.
...
Output: trck_dict(dictionary with number of cylinders and their respective centroid coordinates),
        ...
        cyl(cylinder number), moving(if cylinder is moving then True else False)
User Requirement: 1) Detecting if the cylinder are moving or not Requirements:
...
def track(img,darknet_image,network,class_names,moving):
	...
	try:
		...
		pts = np.array([[0,400],[0,600],[320,600],[320,400]])
		...
		if total_count >0:
			...
			count_frame =0
			total_count =0
		if count_frame > 10 and total_count ==0 :
			moving = False
			count_frame = 0
			total_count = 0
